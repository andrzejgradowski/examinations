...
class NetparDivision
  ...
  def request_with_id
    begin
      uri = URI("#{Rails.application.secrets[:netpar2015_api_url]}/divisions/#{self.id}")
      ...
      Rails.logger.error('======================== API ERROR "/divisions/" ============================')
  ...
  def request_collection
    begin
      case self.category
      when 'R'
        uri = URI("#{Rails.application.secrets[:netpar2015_api_url]}/divisions/ra")
      when 'M'
        uri = URI("#{Rails.application.secrets[:netpar2015_api_url]}/divisions/mor")
      end
